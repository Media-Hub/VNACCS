...
namespace DevComponents.DotNetBar
{
	...
	/// </summary>
    [ToolboxItem(false), DesignTimeVisible(false), DefaultEvent("Click"), Designer(typeof(DevComponents.DotNetBar.Design.SimpleBaseItemDesigner))]
    public class LabelItem : BaseItem, IBlockExtended
    {
        ...
        internal static Color GetTextColor(ItemPaintArgs pa, eDotNetBarStyle effectiveStyle, bool isEnabled, Color defaultColor)
        {
            ...
            {
                if (pa.ContainerControl is Bar && ((Bar)pa.ContainerControl).BarType == eBarType.StatusBar || !(pa.ContainerControl is Bar))
                {
                    // First lookup contextual colors
                    LabelColors lc;
                    if (table.LabelItemColors.TryGetValue(pa.ContainerControl.GetType(), out lc))
