...
namespace DevComponents.DotNetBar
{
	...
	public class ItemStyle: ICloneable
	{
		...
		{
            if (m_CornerType == eCornerType.Rounded && m_CornerDiameter > 1)
            {
                Region reg = System.Drawing.Region.FromHrgn(WinApi.CreateRoundRectRgn(clientRectangle.X, clientRectangle.Y,
                    clientRectangle.Width, clientRectangle.Height, m_CornerDiameter, m_CornerDiameter));
                return reg;
            }
            ...
            else if (m_CornerType == eCornerType.Rounded)
            {
		...
		private GraphicsPath GetPath(Rectangle clientRectangle)
		{
			...
			switch(m_CornerType)
			{
				...
				{
                    path = DisplayHelp.GetRoundedRectanglePath(clientRectangle, m_CornerDiameter);
        ...
        private void AdjustRectangleForBorder(ref Rectangle rPath)
        {
		...
		public void Paint(Graphics g, Rectangle r, string text, Rectangle textRect, System.Drawing.Font font, Point[] borderShape)
		{
			...
			if(g.SmoothingMode==System.Drawing.Drawing2D.SmoothingMode.AntiAlias)
				rPath.Inflate(1,1);
            AdjustRectangleForBorder(ref rPath);
            ...
            Rectangle borderBounds = r; borderBounds.Width--; borderBounds.Height--; AdjustRectangleForBorder(ref borderBounds);
            path = this.GetPath(borderBounds);

			if(m_BackgroundImage!=null)
			{
				Region clip=g.Clip;
                using (GraphicsPath clipBorderPath = this.GetPath(r))
                {
                    g.SetClip(clipBorderPath);
