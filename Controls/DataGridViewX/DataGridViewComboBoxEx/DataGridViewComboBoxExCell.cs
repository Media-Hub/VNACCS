...
namespace DevComponents.DotNetBar.Controls
{
    public class DataGridViewComboBoxExCell : DataGridViewTextBoxCell
    {
        ...
        {
            Debug.WriteLine(e.KeyCode.ToString() + "DataGridViewComboBoxExCell");
            base.OnKeyDown(e, rowIndex);
        }
        protected override bool KeyDownUnsharesRow(KeyEventArgs e, int rowIndex)
        {
            Debug.WriteLine(e.KeyCode.ToString() + " DataGridViewComboBoxExCell KeyDownUnsharesRow");
            return base.KeyDownUnsharesRow(e, rowIndex);
        }
        ...
        {
            get { return (_Col2Width); }
            set { _Col2Width = value; }
        }
            ...
            object initialFormattedValue, DataGridViewCellStyle dataGridViewCellStyle)
        {
            ...
            //tuyenhm thêm dòng này thì mới xổ ra nhiều cột khi ở trên grid được
            ctl.DropDownColumns = cb.DropDownColumns;
            ctl.DropDownWidth = cb.DropDownWidth;           
            ctl.MoRongKhiForcus = cb.MoRongKhiForcus;
            ctl.DropDownColumnsHeaders = cb.DropDownColumnsHeaders;
            ctl.KeyDown += new KeyEventHandler(ctl_KeyDown);
            //ctl.EditingControlWantsInputKey(Keys.Escape, true);  
            ...
            if (this.Value != null)
            {
                (ctl as ComboBox).SelectedValue = this.Value.ToString();
            }

            //Nếu có sự kiện này chạy sẽ rất chậm, đừng bao giờ dùng
            //ctl.SelectedValueChanged += new EventHandler(ctl_SelectedValueChanged);

            ctl.Col1Width = oc.Col1Width;
            ctl.Col2Width = oc.Col2Width;
            ctl.Col3Width = oc.Col3Width;
        }        

        //tuyenhm
        void ctl_KeyDown(object sender, KeyEventArgs e)
        {
            Debug.WriteLine(e.KeyCode.ToString() + " ctl_KeyDown + aaaa");
        }

        //Nếu có sự kiện này chạy sẽ rất chậm, đừng bao giờ dùng
        //void ctl_SelectedValueChanged(object sender, EventArgs e)
        //{
        //    this.DataGridView.Rows[this.RowIndex].Cells[this.ColumnIndex].Value = (sender as DataGridViewComboBoxExEditingControl).SelectedValue;
        //}
