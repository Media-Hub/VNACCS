    ...
    public class MenuPanel : Control, IDesignTimeProvider, IAccessibilitySupport, IKeyTipsControl, IKeyTipsRenderer
    {
        ...
        protected void PaintDotNet(ItemPaintArgs pa)
        {
            ...
            if (m_ParentItem != null)
            {
                ...
                if (BarFunctions.IsOffice2007Style(effectiveStyle) && effectiveStyle != eDotNetBarStyle.Metro)
                {
                    if (this.Region != null)
                    {
                ...
                if (effectiveStyle != eDotNetBarStyle.Metro && ((!BarFunctions.IsOffice2007Style(effectiveStyle) || effectiveStyle == eDotNetBarStyle.Office2010 || effectiveStyle == eDotNetBarStyle.Metro) && m_ParentItem is ButtonItem && m_ParentItem.Displayed && m_ParentItem.Visible && m_ParentItem.Orientation == eOrientation.Horizontal && !(m_ParentItem.ContainerControl is ContextMenuBar)))
                {
        ...
        private bool IsRoundRegion
        {
            get
            {
                if (m_ParentItem != null)
                {
                    if (m_ParentItem is ApplicationButton && m_ParentItem.EffectiveStyle == eDotNetBarStyle.Windows7 || m_ParentItem.EffectiveStyle == eDotNetBarStyle.Metro) return false;
