...
namespace DevComponents.DotNetBar
{
	...
	public partial class SCUAKHAUNN : TableBase
	{
		...
		private string m_CK_TYPE;
		private bool m_CK_TYPEUpdated = false;
		/// <summary>
		/// CK_TYPE.
		/// </summary>
		public string CK_TYPE
		{
			...
			set
			{
				if ((this.m_CK_TYPE != value))
				{
					this.SendPropertyChanging("CK_TYPE");
					this.m_CK_TYPE = value;
					this.SendPropertyChanged("CK_TYPE");
					if ((this.DataStatus != DBStatus.Inserted))
						this.m_CK_TYPEUpdated = true;
				}
			}
		}

		private int? m_CK_HIENTHI = 1;
		private bool m_CK_HIENTHIUpdated = false;
		/// <summary>
		/// CK_HIENTHI.
		/// </summary>
		public int? CK_HIENTHI
		{
			get
			{
				return m_CK_HIENTHI;
		...
		public override string InsertStatement()
		{
			...
			switch(this.DataManagement)
			{
				...
				case DBManagement.SQLLite:
				default:
				sbSQL.Append("INSERT INTO SCUAKHAUNN ([CK_MANUOC], [CK_MACK], [CK_TENCK], [CK_TENCK_VN], [CK_MACKCU], [CK_TENCKCU], [CK_TENBANG], [CK_TYPE], [CK_HIENTHI]) VALUES(").Append("@CK_MANUOC").Append(",").Append("@CK_MACK").Append(",").Append("@CK_TENCK").Append(",").Append("@CK_TENCK_VN").Append(",").Append("@CK_MACKCU").Append(",").Append("@CK_TENCKCU").Append(",").Append("@CK_TENBANG").Append(",").Append("@CK_TYPE").Append(",").Append("@CK_HIENTHI").Append(")");
				break;
		...
		public override string UpdateStatement(string WhereClause)
		{
			...
			switch(this.DataManagement)
			{
				...
				case DBManagement.SQLLite:
				default:
				sbSQL.Append(m_CK_TENCKUpdated ? string.Format(",[CK_TENCK] = {0}", "@CK_TENCK") : string.Empty).Append(m_CK_TENCK_VNUpdated ? string.Format(",[CK_TENCK_VN] = {0}", "@CK_TENCK_VN") : string.Empty).Append(m_CK_MACKCUUpdated ? string.Format(",[CK_MACKCU] = {0}", "@CK_MACKCU") : string.Empty).Append(m_CK_TENCKCUUpdated ? string.Format(",[CK_TENCKCU] = {0}", "@CK_TENCKCU") : string.Empty).Append(m_CK_TENBANGUpdated ? string.Format(",[CK_TENBANG] = {0}", "@CK_TENBANG") : string.Empty).Append(m_CK_TYPEUpdated ? string.Format(",[CK_TYPE] = {0}", "@CK_TYPE") : string.Empty).Append(m_CK_HIENTHIUpdated ? string.Format(",[CK_HIENTHI] = {0}", "@CK_HIENTHI") : string.Empty);
				break;
		...
		public override string UpdateFullStatement(string WhereClause)
		{
			...
			switch(this.DataManagement)
			{
				...
				case DBManagement.SQLLite:
				default:
				sbSQL.AppendFormat("[CK_TENCK] = {0}", "@CK_TENCK").AppendFormat(",[CK_TENCK_VN] = {0}", "@CK_TENCK_VN").AppendFormat(",[CK_MACKCU] = {0}", "@CK_MACKCU").AppendFormat(",[CK_TENCKCU] = {0}", "@CK_TENCKCU").AppendFormat(",[CK_TENBANG] = {0}", "@CK_TENBANG").AppendFormat(",[CK_TYPE] = {0}", "@CK_TYPE").AppendFormat(",[CK_HIENTHI] = {0}", "@CK_HIENTHI");
				break;
		...
		public override List<IDbDataParameter> UpdateParameters()
		{
			...
			paramList.Add(clsDAL.CreateParameter("CK_TYPE", "WChar", clsDAL.ToDBParam(CK_TYPE, ProType.STRING, this.DataManagement) , this.DataManagement));
			paramList.Add(clsDAL.CreateParameter("CK_HIENTHI", "UnsignedTinyInt", clsDAL.ToDBParam(CK_HIENTHI, ProType.NUMBER, this.DataManagement) , this.DataManagement));
			paramList.Add(clsDAL.CreateParameter("CK_MANUOC", "WChar", clsDAL.ToDBParam(CK_MANUOC, ProType.STRING, this.DataManagement) , this.DataManagement));
			paramList.Add(clsDAL.CreateParameter("CK_MACK", "WChar", clsDAL.ToDBParam(CK_MACK, ProType.STRING, this.DataManagement) , this.DataManagement));
			return paramList;
		}
