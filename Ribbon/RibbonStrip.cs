...
namespace DevComponents.DotNetBar
{
    ...
    public class RibbonStrip : ItemControl
    {
        ...
        /// </summary>
        [Editor("DevComponents.DotNetBar.Design.RibbonTabItemGroupCollectionEditor, DevComponents.DotNetBar.Design, Version=11.1.0.0, Culture=neutral,  PublicKeyToken=90f470f34c89ccaf", typeof(System.Drawing.Design.UITypeEditor)), DesignerSerializationVisibility(DesignerSerializationVisibility.Content), Category("Tab Groups"), Browsable(true), DevCoBrowsable(true)]
        ...
        protected override void WndProc(ref Message m)
        {
            if (m.Msg == (int)WinApi.WindowsMessages.WM_NCHITTEST)
            {
                ...
                if (m_CaptionVisible && (effectiveStyle == eDotNetBarStyle.Office2010 || effectiveStyle == eDotNetBarStyle.Metro) && (p.X < 28 && this.RightToLeft == RightToLeft.No || p.X > this.Width - 28 && this.RightToLeft == RightToLeft.Yes) && p.Y < 28)
                {
                    m.Result = new IntPtr((int)WinApi.WindowHitTestRegions.TransparentOrCovered);
                    return;
                }
        ...
        {
            if (this.IsGlassEnabled && (this.EffectiveStyle == eDotNetBarStyle.Office2010 || this.EffectiveStyle == eDotNetBarStyle.Metro))
            {
                    ...
                    if (tab.Group != currentGroup)
                    {
                        if (tab.Visible)
                        {
                            ...
                            if (currentGroup != null)
                            {
                                ...
                                groupRect = new Rectangle(tab.DisplayRectangle.X, y, tab.DisplayRectangle.Width, ((effectiveStyle == eDotNetBarStyle.Office2010 || effectiveStyle == eDotNetBarStyle.Metro) ? tab.DisplayRectangle.Y : tab.DisplayRectangle.Y - 4));
                                groupBounds = new Rectangle(tab.DisplayRectangle.X, y, tab.DisplayRectangle.Width, tab.DisplayRectangle.Bottom - (effectiveStyle == eDotNetBarStyle.Metro ? 1 : m_TabGroupHeight));
                            }
                        }
                    }
        ...
        private int GetCaptionHeight()
        {
            if (m_CaptionHeight == 0)
            {
                ...
                if (this.IsGlassEnabled && EffectiveStyle != eDotNetBarStyle.Office2010 && EffectiveStyle != eDotNetBarStyle.Windows7 && EffectiveStyle != eDotNetBarStyle.Metro)
                    captionHeight += 0;
                    ...
                    captionHeight -= 2;
                else if (!this.IsGlassEnabled && EffectiveStyle == eDotNetBarStyle.Metro)
                    captionHeight -= 2;
        ...
        {
            return GetCaptionHeight() + m_TabGroupHeight + ((this.EffectiveStyle == eDotNetBarStyle.Office2010 || this.EffectiveStyle == eDotNetBarStyle.Windows7 || this.EffectiveStyle == eDotNetBarStyle.Metro) ? 0 : 0);
        }
        ...
        internal BaseItem GetApplicationButton()
        {
            ...
            if ((this.EffectiveStyle == eDotNetBarStyle.Office2010 || this.EffectiveStyle == eDotNetBarStyle.Metro) && this.Items.Count > 0)
            {
                foreach (BaseItem item in this.Items)
                {
